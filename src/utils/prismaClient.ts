...
class CustomPrismaClient {
  ...
  private async findDecryptMiddleware<TResult extends Record<string, any>, TArgs>({args, query,}: {
    args: TArgs;
    ...
    else if (Array.isArray(result)) return result.map((item) => this.decryptQueryDataByKeys(item, tokenEncryptedFields));
  ...
  >({args, query}: { args: TArgs; query: (args: TArgs) => Promise<TResult> }): Promise<TResult> {
    ...
    if (Array.isArray(args.data)) {
      encryptedData = args.data.map((data) => this.encryptQueryDataByKeys(data, tokenEncryptedFields));
    } else {
  ...
  >({args, query}: { args: TArgs; query: (args: TArgs) => Promise<TResult> }): Promise<TResult> {
    ...
    if (Array.isArray(args.create)) {
      encryptedCreateData = args.create.map((data) => this.encryptQueryDataByKeys(data, tokenEncryptedFields));
    } else {
      encryptedCreateData = this.encryptQueryDataByKeys(args.create, tokenEncryptedFields);
    }

    if (Array.isArray(args.update)) {
      encryptedUpdateData = args.update.map((data) => this.encryptQueryDataByKeys(data, tokenEncryptedFields));
    } else {
...
export default new CustomPrismaClient().clientInstance;
